...
export default new Command("hosting:domains:get <siteId> <domainName>")
  ...
  .action(
    async (siteId: string, domainName: string, options): Promise<Domain> => {
      ...
      table.push(["Expected IPs:", domain.provisioning ? domain.provisioning.expectedIps : ""]);
      table.push(["Discovered IPs:", domain.provisioning ? domain.provisioning.discoveredIps : ""]);
      table.push(["Last Check Time:", domain.provisioning ? domain.provisioning.dnsFetchTime : ""]);
